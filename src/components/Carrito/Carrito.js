...
import { BotonPago, CarroVacio, DivCarrito, DivCarritoTotal, H3, H5, HA5, ImagenCarrito, ImagenCarroVacio, ImgUdapteCar, Moneda, PrecioCarrito, SmallCantidad } from './CarritoStyle'
...
import { Cantidad, CantidadNumer, Sumar } from '../productos/detalleProducto/DetalleStyle';
...
const Carrito = () => {
    ...
    let total = 0;
    let subtotal = 0;
    const [isOpenModalActualizar, openModalActualizar, closeModalActualizar] = useModal(false)
    ...
    const handleModal = (id) => {
        let [data] = getProductosById(id, productosCarrito)
    ...
    if (productosCarrito.length === 0) {
        ...
        return (
            <div>
                ...
                <CarroVacio><b>No hay productos</b></CarroVacio>
            </div>
        )
        ...
        <div>
            ...
            <H3>Carrito</H3>
            {
                productosCarrito.map((p, i) => (
                    <div key={i}>
                        <DivCarrito className="carritoProducto" id={p.id} onClick={()=>handleModal(p.id)}>
            ...
            < ActualizarCarrito isOpen={isOpenModalActualizar} closeModal={closeModalActualizar} guardarModal={()=>guardarModal(productoActual,counter)}>               
                <ImgUdapteCar src={productoActual.imagen} alt="imagen Producto" />
                <Moneda>$ {productoActual.precio} <span >MXN</span></Moneda>
